...
import { HeaderNew as HeaderNewUi } from './HeaderNew';
...
const HeaderNew: React.FC = () => {
  ...
  const [searchFormOpen, setSearchFormOpen] = useState<boolean>(!!currentRouteQueryParamFilter?.text?.length);

  const switchUiBookmarkModal = (e: React.FormEvent<HTMLElement>) => {
    e.preventDefault();
    if (!session?.id) {
  ...
  const onSearchInputChange = (e: React.FormEvent<HTMLInputElement>) => {
    ...
    const valueOrUndefined = !value?.length ? undefined : value;

    setSearchValue(valueOrUndefined);
  };
  ...
  useEffect(() => {
    ...
    if (!stringifiedParam?.length) {
      setSearchFormOpen(false);
    }
  }, [currentRoute?.name, currentRouteQueryParamFilter?.text?.toString()]);
  ...
  return (
    <HeaderNewUi
      ...
      onSearchFormLeave={onSearchFormLeave}
    />
  );
};
