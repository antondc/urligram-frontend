...
export const UserVisitor: React.FC<Props> = ({
  userId,
  user,
  createdAt,
  bookmarksIds,
      ...
      <Main>
        <Border grow>
          <Flex horizontal="between">
            ...
            <img className="UserVisitor-image" src={user?.image} />
          </Flex>
          <Hr spacer />
          <Flex>
            <Span className="UserVisitor-detail" size="small">
              Bookmarks:{' '}
          ...
          <Flex>
            {user?.tags?.map((item) => (
              <A className="UserVisitor-tag" href={`tags/${item.name}`} key={item.id} styled={false} frontend>
