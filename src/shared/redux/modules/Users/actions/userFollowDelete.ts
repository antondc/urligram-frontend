...
export const userFollowDelete =
  ({ originUserId, targetUserId }: Params): AppThunk<Promise<Response>, UsersActions> =>
  async (dispatch, getState): Promise<Response> => {
    ...
    try {
      ...
      const { data } = await HttpClient.delete<void, UserFollowDeleteApiResponse>(
        '/users/me/following/' + targetUserId
      );
